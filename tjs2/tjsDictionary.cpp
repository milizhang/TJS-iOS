...
void tTJSDictionaryNI::Assign(iTJSDispatch2 * dsp, bool clear)
{
	...
	else
	{
        ...
        tTJSVariantClosure tmpClosure(&callback, NULL);
		dsp->EnumMembers(TJS_IGNOREPROP, &tmpClosure, dsp);
	...
	iTJSTextWriteStream & stream, const ttstr &indentstr)
{
    ...
    tTJSVariantClosure tmpClosure(&callback, NULL);
	Owner->EnumMembers(TJS_IGNOREPROP, &tmpClosure, Owner);
	...
	std::vector<iTJSDispatch2 *> &stack)
{
		...
		ClassID_Dictionary, (iTJSNativeInstance**)&dicni)) )
	{
		...
		try
		{
			...
			dsp->EnumMembers(TJS_IGNOREPROP, new tTJSVariantClosure(&callback, NULL), dsp);
		}
		catch(...)
		{
			stack.pop_back();
